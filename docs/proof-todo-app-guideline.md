...
Map that to `verification_status`: `verified` if `task_shown && confidence ≥ threshold && !is_screenshot_or_screen`, else `rejected` (with the reason surfaced to the user). Having Gemini flag screenshots/screens is what plugs the hole Layer A can't — it's why allowing gallery is workable.
